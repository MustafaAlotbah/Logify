...
#define LOGIFY_SCOPED_LOGGER() \
    Logify::ScopedLogger LOGIFY_CONCATENATE(scopedLogger_, __COUNTER__)(logger, LOGIFY_FUNC_SIGNATURE, Logify::LogLevel::INFO)

// Macro to create a unique ScopedLogger with a specified log level
#define LOGIFY_SCOPED_LOGGER_LEVEL(level) \
    Logify::ScopedLogger LOGIFY_CONCATENATE(scopedLogger_, __COUNTER__)(logger, LOGIFY_FUNC_SIGNATURE, level)
